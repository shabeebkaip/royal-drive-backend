...
// Ensure slug is unique only when it's a non-null string (avoid duplicate null collisions)
VehicleSchema.index(
  { 'marketing.slug': 1 },
...
// Generate slug from make, model, year, and _id suffix (no stock number)
VehicleSchema.pre('save', async function(this: IVehicle, next) {
  // Always generate slug if it doesn't exist or if related fields changed
  if (!this.marketing.slug || this.isModified('make') || this.isModified('model') || this.isModified('year')) {
  ...
  // Use last 6 chars of _id to ensure uniqueness without stock number
  const idSuffix = String(this._id).slice(-6);
  const slug = `${this.year}-${makeSlug}-${modelSlug}-${idSuffix}`
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '-')
      .replace(/-+/g, '-')
      .replace(/^-|-$/g, '');
